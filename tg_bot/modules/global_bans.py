...
from telegram import Message, Update, User, Chat, InlineKeyboardMarkup
...
async def gban(update: Update, context, args: List[str] = None):
    if args is None:
    ...
    keyboard = []
    await message.reply_text(starting, reply_markup=keyboard, parse_mode=ParseMode.HTML)
    ...
    send_to_list(context.bot, SUDO_USERS + SUPPORT_USERS,
                   ...
                   html=True)
    await message.reply_text(gban_complete, reply_markup=keyboard, parse_mode=ParseMode.HTML)

async def ungban(update: Update, context, args: List[str] = None):
    if args is None:
        args = context.args or []
    message = update.effective_message
...
async def gbanlist(update: Update, context):
...
async def enforce_gban(update: Update, context):
    if sql.does_chat_gban(update.effective_chat.id) and (await update.effective_chat.get_member(context.bot.id)).can_restrict_members:
        user = update.effective_user
...
@user_admin
async def gbanstat(update: Update, context, args: List[str] = None):
    if args is None:
        args = context.args or []
    if len(args) > 0:
