...
@user_admin
@loggable
async def del_joined(update: Update, context, args: List[str] = None) -> str:
    if args is None:
    ...
    if not args:
        ...
        if del_pref:
            await update.effective_message.reply_text("I should be deleting `user` joined the chat messages now.")
        else:
            await update.effective_message.reply_text("I'm currently not deleting old joined messages!")
        return ""

    if args[0].lower() in ("on", "yes"):
        sql.set_del_joined(str(chat.id), True)
        await update.effective_message.reply_text("I'll try to delete old joined messages!")
        return "<b>{}:</b>" \
        ...
        await update.effective_message.reply_text("I won't delete old joined messages.")
        return "<b>{}:</b>" \
               "\n#CLEAN_SERVICE_MESSAGE" \
               "\n<b>Admin:</b> {}" \
...
async def delete_join(update: Update, context):
    chat = update.effective_chat
        ...
        for new_mem in new_members:
            if new_mem.id == OWNER_ID:
                ...
                if welc_type != sql.Types.TEXT and welc_type != sql.Types.BUTTON_TEXT:
                    await _get_enum_func(welc_type)(chat.id, cust_welcome)
                    return
                ...
                sent = await send(update, res, keyboard,
                                  sql.DEFAULT_WELCOME.format(first=first_name))
            await delete_join(update, context)

        prev_welc = sql.get_clean_pref(chat.id)
        if prev_welc:
            try:
                await context.bot.delete_message(chat.id, prev_welc)
            except BadRequest as excp:
                pass
...
async def left_member(update: Update, context):
    chat = update.effective_chat
    ...
    if should_goodbye:
        ...
        if left_mem:
            ...
            if goodbye_type != sql.Types.TEXT and goodbye_type != sql.Types.BUTTON_TEXT:
                await _get_enum_func(goodbye_type)(chat.id, cust_goodbye)
                return
            ...
            await send(update, res, keyboard, sql.DEFAULT_GOODBYE)
            await delete_join(update, context)


@user_admin
async def welcome(update: Update, context, args: List[str] = None):
    if args is None:
        args = context.args or []
    chat = update.effective_chat
    if len(args) == 0 or args[0].lower() == "noformat":
        ...
        if welcome_type == sql.Types.BUTTON_TEXT:
            ...
            if noformat:
                await _get_enum_func(welcome_type)(chat.id, welcome_m)
...
@user_admin
async def goodbye(update: Update, context, args: List[str] = None):
    if args is None:
        args = context.args or []
    chat = update.effective_chat
...
@user_admin
@loggable
async def set_welcome(update: Update, context) -> str:
    chat = update.effective_chat
    user = update.effective_user
...
@user_admin
@loggable
async def reset_welcome(update: Update, context) -> str:
    chat = update.effective_chat
    user = update.effective_user
...
@user_admin
@loggable
async def set_goodbye(update: Update, context) -> str:
    chat = update.effective_chat
    user = update.effective_user
    msg = update.effective_message
    text, data_type, content, buttons = get_welcome_type(msg)
...
@user_admin
@loggable
async def reset_goodbye(update: Update, context) -> str:
    chat = update.effective_chat
    user = update.effective_user
...
@user_admin
@loggable
async def clean_welcome(update: Update, context, args: List[str] = None) -> str:
    if args is None:
        args = context.args or []
    chat = update.effective_chat
    user = update.effective_user
...
WELC_HELP_TXT = "Your group's welcome/goodbye messages can be personalised in multiple ways. If you want the messages" \
                ...
                "replying to the desired media, and calling /setwelcome.".format("{" + "bot_username}")


@user_admin
async def welcome_help(update: Update, context):
...
DEL_JOINED = CommandHandler("rmjoin", del_joined, filters=filters.ChatType.GROUPS)
...
application.add_handler(DEL_JOINED)
