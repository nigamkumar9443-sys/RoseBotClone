...
from tg_bot import application, BAN_STICKER
...
async def warn(user: User, chat: Chat, reason: str, message: Message, warner: User = None) -> str:
    ...
    if num_warns >= limit:
        ...
        await message.bot.send_sticker(chat.id, BAN_STICKER)
        keyboard = []
        log_reason = "<b>{}:</b>" \
                     "\n#WARN_BAN" \
                     "\n<b>Admin:</b> {}" \
...
@bot_admin
@loggable
async def button(update: Update, context) -> str:
...
@can_restrict
@loggable
async def warn_user(update: Update, context, args: List[str] = None) -> str:
    if args is None:
...
@bot_admin
@loggable
async def reset_warns(update: Update, context, args: List[str] = None) -> str:
    if args is None:
        args = context.args or []
    message = update.effective_message
    chat = update.effective_chat
...
async def warns(update: Update, context, args: List[str] = None):
    if args is None:
        args = context.args or []
    message = update.effective_message
    chat = update.effective_chat
...
@user_admin
async def add_warn_filter(update: Update, context):
    chat = update.effective_chat
...
@user_admin
async def remove_warn_filter(update: Update, context):
    chat = update.effective_chat
    msg = update.effective_message
...
async def list_warn_filters(update: Update, context):
    chat = update.effective_chat
...
@loggable
async def reply_filter(update: Update, context) -> str:
    chat = update.effective_chat
    message = update.effective_message
...
@user_admin
@loggable
async def set_warn_limit(update: Update, context, args: List[str] = None) -> str:
    if args is None:
        args = context.args or []
    chat = update.effective_chat
    user = update.effective_user
...
@user_admin
async def set_warn_strength(update: Update, context, args: List[str] = None):
    if args is None:
        args = context.args or []
    chat = update.effective_chat
    user = update.effective_user
